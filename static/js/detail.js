window.addEventListener('DOMContentLoaded', function() {
    ...
    function loadMoreComments() {
        ...
        fetch(`/photos/load-more-comments/?photo_id=${photoId}&offset=${currentOffset}`, {
            headers: {
                'X-Requested-With': 'XMLHttpRequest'
            }
        })
    ...
    document.addEventListener('click', function(e) {
        ...
        if (e.target.closest('.comment-action-btn') && !e.target.closest('.reply-btn')) {
            ...
            fetch(`/photos/comment/${commentId}/like/`, {
                method: 'POST',
                headers: {
                    'X-CSRFToken': document.querySelector('[name=csrfmiddlewaretoken]').value,
                    'X-Requested-With': 'XMLHttpRequest'
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (commentForm) {
        commentForm.addEventListener('submit', function(e) {
            ...
            let url = `/photos/photo/${photoId}/comment/`;
            if (parentId) {
        ...
        commentContent.addEventListener('input', function(e) {
            ...
            if (atPos !== -1) {
                ...
                if (mentionSearchTerm.length > 0) {
                    fetch(`/photos/search-users/?q=${encodeURIComponent(mentionSearchTerm)}`, {
                        headers: {
                            'X-Requested-With': 'XMLHttpRequest'
                        }
                    })
